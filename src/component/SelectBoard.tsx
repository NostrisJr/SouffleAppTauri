...
function SelectBoard({ boards, selectedDevice, setSelectedDevice, debuggingMode }: selectPortProps) {
    ...
    <div className="relative z-10 p-2" onMouseLeave={() => setClicked(false)}>
      ...
      >
        {boards.map((board, index) => (
          <PortItem key={index} device={board} handleClick={handleClick} debuggingMode={debuggingMode} />
        ))}
      </ul>
    </div>
  );
