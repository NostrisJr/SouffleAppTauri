...
async function formateArduinoCliConfig() {
    ...
    const defaultConfigPath = await path.resolveResource("Resources/Arduino15/arduino-cli.yaml");
    ...
    if (configExists) {
        try {
            ...
            logError("An error occurred while removing existing arduino-cli config : " + err)
        }
    }
    ...
    try {
        ...
        logMessage("New arduino-cli.yaml sucessfully writen with paths :\n data : " + dataPath + "\ndownloads : " + downloadsPath + "\nuser : " + userPath)
    } catch (err) {
...
export async function checkResourcesDirectory(setDisabled: React.Dispatch<React.SetStateAction<boolean>>) {
    const fs = await import("@tauri-apps/api/fs")
    const path = await import("@tauri-apps/api/path")
    ...
    try {
        ...
        if (resourcesAppDirExists) {
            try {
                ...
                logError("An error occurred while removing Resources Directory from App Data. Directory probably still incompletly removed : " + err)
            }
