...
function PresetItem({ label, handleClick }: presetItemProps) {
  ...
  async function deletePresetLocal() {
    ...
    const confirmed = await dialogModule.confirm("Are you sure to delete this preset ?\n This action cannot be reverted", { title: "Souffle - Desktop", type: "warning", okLabel: "Delete" })
    if (confirmed) {
      try {
  ...
  return (
    <li className="">
      <div className="flex items-center">
        ...
        <button className="flex text-s-white w-full hover:text-s-purple text-left align-middle h-max overflow-clip text-ellipsis whitespace-nowrap" onClick={() => handleClick(label)}>
          {label}
        </button>

          <button
            className={`${label==="Default" ? "fill-s-bg-light" : "fill-s-bg-dark hover:fill-s-purple relative after:absolute after:right-10 after:bottom-5 after:shadow-lg after:z-10 after:block after:rounded-lg after:bg-s-bg-light after:text-2xs after:text-s-bg-dark after:text-body after:opacity-0 after:transition after:duration-200 after:content-[''] hover:after:px-2 hover:after:py-1 hover:after:opacity-100 hover:after:delay-500 after:whitespace-nowrap hover:after:content-['Delete_Preset']" } flex justify-center items-center w-12 h-12 rounded-full`}
            onClick={() => deletePresetLocal()}
            disabled={label==="Default"}
          >
            <svg width="20" height="20" fill="inherit" viewBox="0 0 576 512" xmlns="http://www.w3.org/2000/svg">
...
function SelectPreset({ selectedPreset, setSelectedPreset, setValues, disabled }: selectPresetProps) {
  ...
  return (
    <div className="relative z-0" onMouseLeave={() => setClicked(false)}>
      <button className="text-s-purple font-body text-lg flex justify-start gap-4 items-baseline disabled:text-bg-s-dark"
      ...
      <ul
        className={`${clicked === false ? "hidden" : "flex shadow-md"} w-full max-h-[30em] overflow-y-auto overflow-x-clip text-ellipsis flex-col bg-s-bg-light absolute z-10 rounded-md divide-s-bg-dark divide-y px-4`}
      >
